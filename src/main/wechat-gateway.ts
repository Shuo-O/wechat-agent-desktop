...
export class WechatGateway {
  ...
  async beginLogin(force = false): Promise<void> {
    ...
    const baseUrl = data.wechat.credentials?.baseUrl || DEFAULT_BASE_URL;
    ...
    try {
      while (true) {
        ...
        let lastStatus = "";
        while (true) {
          const status = await fetchQrStatus(baseUrl, qr.qrcode);
  ...
  async sendReplyText(contactId: string, contextToken: string, text: string): Promise<void> {
    const credentials = this.store.getData().wechat.credentials;
    if (!credentials) {
  ...
  async sendTyping(contactId: string, contextToken: string): Promise<void> {
    ...
    await sendTyping(credentials.baseUrl, credentials.token, contactId, ticket, 1);
  }
}
